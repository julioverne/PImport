...
@interface PImportController : PSListController {
	UILabel* _label;
	UILabel* underLabel;
}
- (void)HeaderCell;
...
@implementation PImportController
- (id)specifiers {
	if (!_specifiers) {
		...
		[spec setProperty:@"Silent Import: Will Import Uploaded Photos Immediately Without Edit Tag Alert." forKey:@"footerText"];
        [specifiers addObject:spec];
		spec = [PSSpecifier preferenceSpecifierNamed:@"Silent Import"
                                                  target:self
                                                  ...
                                                  detail:Nil
											        cell:PSSwitchCell
											        edit:Nil];
		[spec setProperty:@"SilentImport" forKey:@"key"];
...
- (void)love
{
	...
	[twitter setInitialText:@"#PImport by @ijulioverne is cool!"];
	if (twitter != nil) {
...
- (void)reset
{
	...
	notify_post("com.julioverne.pimport/Settings");
}
...
- (void)HeaderCell
{
	...
	CGRect frame = CGRectMake(0, 20, width, 60);
		...
		[underLabel setText:@"Photo Importer"];
